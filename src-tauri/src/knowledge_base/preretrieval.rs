...
use crate::config::{AgentConfig, AppConfig, KnowledgeBaseSettings};
...
pub(crate) async fn build_kb_context_item(
    ...
    let kb_settings = KnowledgeBaseSettings::from(&config.rag);
    if !kb_settings.enabled || kb_settings.knowledge_bases.is_empty() {
        return None;
    }
