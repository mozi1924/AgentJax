...
use crate::config::{AppConfig, KnowledgeBaseEntry, KnowledgeBaseSettings};
use crate::error::{AgentJaxError, AgentJaxResult};
use crate::knowledge_base::types::{
    HybridSearchResult, IndexingProgress, KnowledgeBaseInfo,
};
...
use crate::rag::chunking::{Chunker, MarkdownChunker};
use crate::rag::fts_store::{FtsStore, content_hash};
use crate::rag::types::{
    Chunk, Document, SearchConfig,
};
...
impl KnowledgeBaseManager {
    ...
    pub fn is_embedding_disabled(&self) -> bool {
        self.engine.is_embedding_disabled()
    }
    ...
    pub async fn index_document(
        ...
        let embeddings = self.engine.embed_batch(_app_config, &texts).await?;
        ...
        kb.fts_store.insert_chunks(&embedded_chunks, self.engine.embedding_model())?;
    ...
    async fn embed_batch_with_retry(
        ...
        match engine.embed_batch(app_config, &texts).await {
            Ok(embeddings) => {
                Self::commit_batch(kb, batch, &chunk_ids, &embeddings).await?;
                return Ok(());
            }
        ...
        if batch.len() > MIN_BATCH {
            ...
            Box::pin(Self::embed_batch_with_retry(
                kb, app_config, engine, &batch[..mid],
            ))
            .await?;
            Box::pin(Self::embed_batch_with_retry(
                kb, app_config, engine, &batch[mid..],
    ...
    pub async fn search(
        ...
        let maybe_query_vector = if self.engine.is_embedding_disabled() {
            log::debug!("Embedding disabled — using FTS5-only search");
            None
        } else {
            match self.engine.embed_single(app_config, query).await {
