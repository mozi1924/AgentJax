...
pub async fn stream_response_with_behavior(
    ...
    let first_attempt = if use_sse {
        create_response_streaming_sse(resolved, req, behavior, persistence, cancel_rx, on_delta)
            .await
    } else {
    ...
    if behavior.retry_store_false && should_retry_with_store_false(&first_attempt, persistence) {
        let store_false_attempt = if use_sse {
            create_response_streaming_sse(resolved, req, behavior, false, cancel_rx, on_delta)
                .await
        } else {
            create_response_streaming_websocket(
