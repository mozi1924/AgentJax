...
pub fn build_recovery_developer_note(conversation_id: &str) -> crate::error::AgentJaxResult<Option<Value>> {
    with_conversation_lock(conversation_id, || {
        ...
        let interruption_reason = if unresolved.is_empty() && has_assistant_draft {
            ...
            "The assistant finished its last tool call but never produced a final response."
                .to_string()
        };
